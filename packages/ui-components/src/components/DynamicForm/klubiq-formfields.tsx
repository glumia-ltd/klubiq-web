...
export const KlubiqFormFields = ({ field, formatters }: FormFieldsProps) => {
        ...
        >
          <FormikConsumer>
            {({ values }) => (
              <>
                {field.fields.map((subField, index) => {
                  const shouldShow = !subField.showIf || subField.showIf(values);
                  if (!shouldShow) return null;
                  return (
                    <KlubiqFormFields
                      key={`${subField.name}-${index}`}
  ...
  const getInputProps = (formikField: any, meta: any) => {
    ...
    if (field.adornment) {
      baseProps.InputProps = {
        ...baseProps.InputProps,
        startAdornment: field.adornment.prefix && (
          <InputAdornment position='start'>{field.adornment.prefix}</InputAdornment>
        ),
        endAdornment: field.adornment.suffix && (
          <InputAdornment position='end'>{field.adornment.suffix}</InputAdornment>
        ),
      };
    }
  ...
  const renderInfoMessage = (formikField: any) => {
    ...
    const shouldShow = !field.infoMessage.showIf || field.infoMessage.showIf(formikField.value);
    if (!shouldShow) return null;
        ...
        case 'select':
          return (
            <Select
              ...
              label={field.isInFieldLabel && field.label}
              multiple={field.multiple}
              value={field.readonly ? field.predefinedValue : formikField.value || null}
              disabled={field.disabled}
            >
              {(() => {
                const options =
